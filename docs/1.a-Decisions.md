...
### Returning Partial Character Responses

After every mutating API call, the server responds partial responses and not the full character
data, unless the client performs a full character request.

While this makes building the response details a bit more work. The API as a whole becomes less ambiguous, and lets the
client focus on changing only what's needed.
